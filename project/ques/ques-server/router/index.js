...
router.all('*', (req, res, next) => {
  // next();return;
  if (['/users/login', '/users/add','/users/isexist'].indexOf(req.path) >= 0) {
    next()
  } else {
...
// router.get('/classes/resetpassword', Classes.resetpassword)
router.get('/classes/isexist', Classes.isexist)
// router.post('/classes/login', Classes.login)
// router.get('/classes/status', Classes.status)
